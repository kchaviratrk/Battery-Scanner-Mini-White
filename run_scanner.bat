...
echo ====================================================
echo Checking for results in: c:\Battery-Scanner-Mini-White\results\
echo.
if exist "c:\Battery-Scanner-Mini-White\results\scan_results.json" (
    echo JSON results found: scan_results.json
    for %%I in ("c:\Battery-Scanner-Mini-White\results\scan_results.json") do echo File size: %%~zI bytes
    echo.
    set /p open="Open JSON results? (y/N): "
    if /i "%open%"=="y" start notepad "c:\Battery-Scanner-Mini-White\results\scan_results.json"
) else (
    echo No JSON results found
)

if exist "c:\Battery-Scanner-Mini-White\results\scan_results.csv" (
    echo CSV results found: scan_results.csv
    for %%I in ("c:\Battery-Scanner-Mini-White\results\scan_results.csv") do echo File size: %%~zI bytes
    echo.
    set /p open_csv="Open CSV results? (y/N): "
    if /i "%open_csv%"=="y" start notepad "c:\Battery-Scanner-Mini-White\results\scan_results.csv"
) else (
    echo No CSV results found
)

echo.
echo All files in results directory:
dir "c:\Battery-Scanner-Mini-White\results\" /b 2>nul || echo Directory is empty

if not exist "c:\Battery-Scanner-Mini-White\results\scan_results.json" if not exist "c:\Battery-Scanner-Mini-White\results\scan_results.csv" (
    echo.
    echo No results found - run scanner first (option 3)
)
pause
goto MENU
