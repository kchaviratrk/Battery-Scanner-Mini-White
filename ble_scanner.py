...
def check_pc_ble_driver_py_version():
    ...
    try:
        ...
        if major_minor in compat_map:
            fw_version = compat_map[major_minor]
            print(f"pc-ble-driver-py version: {version_str} (OK, requires {fw_version})")
        else:
            print(f"WARNING: pc-ble-driver-py version {version_str} compatibility unknown.")
            print("Supported versions:")
            for ver, fw in compat_map.items():
                print(f"  v{ver} -> {fw}")
        ...
        print(f"WARNING: Could not check pc-ble-driver-py version: {e}")
...
def validate_environment():
    ...
    print("Environment check passed. Starting BLE scanner...")
    print()
...
def ManufEvent(qrCode, failure_code, command):
    """Send manufacturing event to traceability database"""
    try:
        timezone = pytz_timezone('US/Central')
        startTime = datetime.now(timezone).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
        
        payload = {
            "Id": 0,
            "serialNumber": qrCode,
            "stationName": "BLE Functionality Test",
            "startTime": startTime,
            "endTime": startTime,
            "elapsedTime": 30,
            "failureDescription": failure_code,
            "comment": command,
            "operator": "System"
        }
        
        response = requests.post(
            "http://20.97.201.175:6699/addManufEvent",
            json=payload,
            headers={'Content-Type': 'application/json'}
        )
        
        if response.status_code == 200:
            print("Manufacturing event logged successfully")
        else:
            print(f"Failed to log manufacturing event: {response.status_code}")
            
    except Exception as e:
        print(f"Error logging manufacturing event: {e}")
            ...
            if battery_parsed and battery is not None:
                ...
                try:
                    ...
                    if qr:
                        ...
                        ManufEvent(qr, failure_code, comment)
                except Exception as e:
...
def run_multi_scan(targets: Dict[str, str], timeout_s: int) -> Dict[str, Dict]:
    ...
    ble_driver = None
    try:
        ble_driver = initialize_driver_multi(COM_PORT, targets, results, pending)
        start = perf_counter()
...
def BLE_scanning(formatted_mac, qrcode):
    ...
    try:
        ...
        ble_driver = initialize_driver(COM_PORT, formatted_mac, qrcode)
    ...
    try:
        ...
        if raw_battery:
            ...
            try:
                ...
                ManufEvent(qrcode, failure_code, device_result['comment'])
            except Exception as e:
                print(f"Error sending events: {e}")

        else:
            device_result['comment'] = 'No data obtained'
            ManufEvent(qrcode, 'SCAN-FAIL-001', 'BLE scan failed')
...
def process_mac_list(mac_list: List[str]):
    ...
    4) Wait until all MACs processed or timeout
    ...
    try:
        ...
        # Dynamic timeout based on batch size - more time for larger batches
        if total <= 50:
            overall_timeout = config.SCAN_TIMEOUT * 2  # Normal timeout
        elif total <= 500:
            overall_timeout = config.SCAN_TIMEOUT * 5  # 5x timeout for medium batches
        elif total <= 2000:
            overall_timeout = config.SCAN_TIMEOUT * 10  # 10x timeout for large batches
        else:
            overall_timeout = config.SCAN_TIMEOUT * 20  # 20x timeout for massive batches (14k+)
        
        deadline = time.time() + overall_timeout

        last_print = 0
        print_interval = 10 if total > 1000 else 5  # Less frequent prints for large batches
        
        print(f"Timeout set to {overall_timeout}s for {total} devices")
        
        # Wait until pending empty or timeout
        checkpoint_interval = max(100, total // 50)  # Checkpoint every 100 devices or 2% of total
        last_checkpoint = 0
        ...
        while pending and time.time() < deadline:
            ...
            # Incremental checkpoint saving for large batches
            ...
            if now - last_print > print_interval:
                ...
                # Estimate completion time for large batches
                if processed_count > 10 and total > 1000:
                    rate = processed_count / elapsed_so_far
                    remaining_time = len(pending) / rate if rate > 0 else 0
                    print(f"Processing rate: {rate:.2f} devices/second")
                    print(f"Estimated time remaining: {remaining_time:.1f}s ({remaining_time/60:.1f} minutes)")
                
                last_print = now
            
            # Shorter sleep for large batches to be more responsive
            sleep_time = 0.1 if total > 1000 else 0.5
...
class DiscoveryObserver(BLEDriverObserver):
    ...
    def __init__(self, ble_driver, min_rssi_threshold=-200):  # Changed default to capture all devices
        super().__init__()
        self.ble_driver = ble_driver
        self.discovered_devices = {}
        self.last_print_time = 0
        self.min_rssi_threshold = min_rssi_threshold  # Filter by distance

    def on_gap_evt_adv_report(self, ble_driver, conn_handle, peer_addr, rssi, adv_type, adv_data):
        # Filter by RSSI (distance) first
        if rssi < self.min_rssi_threshold:
            return  # Device too far away, ignore
        ...
        # Parse advertising data for device name and manufacturer data
        for data_type, adv_payload in adv_data.records.items():
            ...
            # Check for manufacturer-specific data (potential battery info)
            elif "manufacturer_specific_data" in data_type_str:
                device_info['has_battery_data'] = True
                # Try to parse battery data using universal protocol
                try:
                    # Protocol 1: Complex format
                    if len(adv_payload) == 26 and adv_payload[-2] == 179 and adv_payload[-3] == 255:
                        battery_int = float(adv_payload[-1])
                        battery_dec = float(adv_payload[-4]) / 100
                        battery = battery_int + battery_dec
                        device_info['battery_voltage'] = battery
                    # Protocol 2: Simple format  
        ...
        # Print discovery in real-time (throttled)
        current_time = time.time()
        if current_time - self.last_print_time > 2:  # Print every 2 seconds
            count = len(self.discovered_devices)
            print(f"Discovered {count} devices so far...")
            self.last_print_time = current_time


def discover_nearby_devices(scan_duration: int = 15, scan_all_rssi: bool = True) -> List[str]:
    """Discover nearby BLE devices and return a list of MACs
    
    Args:
        scan_duration: How long to scan for devices (seconds)
        scan_all_rssi: If True, scan all devices regardless of RSSI (default: True)
    
    Returns:
        List of MAC addresses of discovered devices
    """
    from config import config
    
    if scan_all_rssi:
        # Skip user prompt and scan all devices
        distance_label = "No limit"
        rssi_threshold = -200
        print(f"Scanning for {scan_duration} seconds for ALL BLE devices (no RSSI filter)...")
    else:
        # Use existing distance selection menu
        print(f"Select maximum distance for device discovery:")
        print("1. ~1 meter (RSSI > -45 dBm)")
        print("2. ~2 meters (RSSI > -55 dBm)")  
        print("3. ~3 meters (RSSI > -65 dBm)")
        print("4. ~5 meters (RSSI > -75 dBm)")
        print("5. ~10+ meters (RSSI > -85 dBm)")
        print("6. No distance filter (All devices - RECOMMENDED)")
        print("Enter choice (1-6, default: 6 for all RSSI values):")
        
        distance_choice = input().strip()
        distance_map = {
            "1": ("1m", -45),
            "2": ("2m", -55), 
            "3": ("3m", -65),
            "4": ("5m", -75),
            "5": ("10m", -85),
            "6": ("No limit", -200)  # Very low threshold = no filtering
        }
        
        if distance_choice in distance_map:
            distance_label, rssi_threshold = distance_map[distance_choice]
        else:
            distance_label, rssi_threshold = ("No limit", -200)  # Default to ALL devices
        
        if distance_label == "No limit":
            print(f"Scanning for {scan_duration} seconds for ALL BLE devices (no distance filter)...")
        else:
            print(f"Scanning for {scan_duration} seconds for BLE devices within ~{distance_label} (RSSI > {rssi_threshold} dBm)...")
    ...
    try:
        # Initialize driver for discovery
        ble_driver = BLEDriver(serial_port=COM_PORT, baud_rate=1000000)
        ...
        # Start scanning with no timeout (we control it)
        scan_params = BLEGapScanParams(interval_ms=100, window_ms=50, timeout_s=0)
        ble_driver.ble_gap_scan_start(scan_params)
        ...
        sorted_devices = sorted(discovered_devices.items(), 
                               key=lambda x: x[1]['rssi'], reverse=True)
        
        if distance_label == "No limit":
            print(f"\nDiscovered {len(sorted_devices)} BLE devices (no distance filter):")
        else:
            print(f"\nDiscovered {len(sorted_devices)} BLE devices within ~{distance_label}:")
        print("-" * 90)
        print(f"{'#':<3} {'MAC Address':<18} {'RSSI':<6} {'Distance':<10} {'Name':<20} {'Battery':<10}")
        print("-" * 90)
        
        def rssi_to_distance_estimate(rssi):
            """Convert RSSI to rough distance estimate"""
            if rssi > -45:
                return "~0.5-1m"
            elif rssi > -55:
                return "~1-2m"
        ...
        for idx, (mac, info) in enumerate(sorted_devices, 1):
            name = info['name'][:18] if len(info['name']) > 18 else info['name']
            battery_str = f"{info['battery_voltage']:.2f}V" if info['battery_voltage'] else "N/A"
            distance_est = rssi_to_distance_estimate(info['rssi'])
            print(f"{idx:<3} {mac:<18} {info['rssi']:<6} {distance_est:<10} {name:<20} {battery_str:<10}")
            
            # Show only first 100 for readability, but process all devices
            if idx >= 100:
                remaining_count = len(sorted_devices) - idx
                print(f"... and {remaining_count} more devices (showing first 100 for readability)")
                print(f"Total discovered devices: {len(sorted_devices)}")
                break
        
        print("-" * 90)
        
        # Ask user which devices to process
        print("\nSelect devices to process:")
        print(f"Total devices discovered: {len(sorted_devices)}")
        print("Enter device numbers (1-100), separated by commas (e.g., 1,3,5)")
        print("Or enter:")
        print("  'all' - process ALL discovered devices (no limit)")
        print("  'battery' - process devices with battery data only (no limit)")
        print("  'good-battery' - process devices with battery >= 2.9V only")
        print("  'low-battery' - process devices with battery < 2.9V only")
        print(f"  'first-N' - process first N devices (e.g., 'first-5000')")
        print(f"Note: Current MAX_QR_BATCH setting ({config.MAX_QR_BATCH}) will be ignored for 'all' and 'battery' selections")
        
        selection = input().strip().lower()
        
        if selection == 'all':
            discovered_macs = [mac for mac, _ in sorted_devices]  # Process ALL devices, no limit
            print(f"Processing ALL {len(discovered_macs)} devices")
        elif selection == 'battery':
            discovered_macs = [mac for mac, info in sorted_devices 
                             if info['has_battery_data']]  # Process all with battery data, no limit
            print(f"Processing {len(discovered_macs)} devices with battery data")
        elif selection == 'good-battery':
            discovered_macs = [mac for mac, info in sorted_devices 
                             if info['has_battery_data'] and info.get('battery_voltage', 0) >= 2.9]
            print(f"Processing {len(discovered_macs)} devices with good battery (>=2.9V)")
        elif selection == 'low-battery':
            discovered_macs = [mac for mac, info in sorted_devices 
                             if info['has_battery_data'] and info.get('battery_voltage', 0) < 2.9]
            print(f"Processing {len(discovered_macs)} devices with low battery (<2.9V)")
        elif selection.startswith('first-'):
            try:
                count = int(selection.split('-')[1])
                count = min(count, len(sorted_devices))  # Remove MAX_QR_BATCH limitation
                discovered_macs = [mac for mac, _ in sorted_devices[:count]]
                print(f"Processing first {count} devices")
            except (ValueError, IndexError):
                print("Invalid 'first-N' format. Use 'first-1000' for example.")
                return []
        else:
            ...
            try:
                ...
                for num in selected_nums:
                    if 1 <= num <= len(sorted_devices):
                        mac = sorted_devices[num-1][0]
                        discovered_macs.append(mac)
                    # Remove MAX_QR_BATCH limitation for manual selection
            except ValueError:
                print("Invalid selection format")
                return []
        
        if discovered_macs:
            print(f"Selected {len(discovered_macs)} devices for processing")
        else:
...
# Update main to accept batch input and manual comma-separated list
def main():
    """Main scanner loop with batch processing support"""
    init()  # Initialize colorama
    setup_logging()

    print("BLE Universal Scanner - Batch Mode")
    print("=" * 60)

    try:
        # Offer input options
        print("Enter 'file' to load QR/MACs from file, 'manual' to paste comma-separated list, 'discover' to scan for nearby devices, or 'exit' to quit:")
        ...
        mac_list = []
        if choice == 'file':
            print(f"Enter file path (default: {config.QR_INPUT_FILE}):")
            path = input().strip() or config.QR_INPUT_FILE
            mac_list = load_qr_codes(path)
        elif choice == 'manual':
            print("Paste comma-separated QR/MAC list:")
            line = input().strip()
            ...
            for t in tokens:
                if t not in seen:
                    seen.add(t)
                    out.append(t)
                if len(out) >= config.MAX_QR_BATCH:
                    break
            mac_list = out
        elif choice == 'discover':
            print("Scanning for nearby BLE devices...")
            print("Enter scan duration in seconds (default: 15, max: 3600 for 1 hour):")
            duration_input = input().strip()
            try:
                scan_duration = int(duration_input) if duration_input else 15
                scan_duration = max(5, min(3600, scan_duration))  # Limit between 5 seconds - 1 hour
                print(f"Scan duration set to: {scan_duration} seconds ({scan_duration/60:.1f} minutes)")
            except ValueError:
                scan_duration = 15
                print(f"Invalid input, using default: {scan_duration} seconds")
            discovered_devices = discover_nearby_devices(scan_duration)
            if not discovered_devices:
                print("No devices found nearby")
                return
            mac_list = discovered_devices
        else:
        ...
        print(f"Starting batch processing: {len(mac_list)} items (max {config.MAX_QR_BATCH})")
        # If post-test flow is enabled, run double-scan (pre/post).
        if config.POST_TEST_ENABLED:
            perform_double_scan(mac_list, config.SCAN_TIME)
        else:
            process_mac_list(mac_list)
